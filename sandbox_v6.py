...
def neural_network(network_architecture: list):
    ...
    def forward_axons_and_dentrites_update(neurons_activation, neurons_stress, layer_axons, layer_dentrites):
        presynaptic_negative_values = (cp.dot(neurons_activation, layer_axons)) < 0
        activation_negative_values = neurons_activation < 0
        indices_to_be_nudge_up = cp.dot(activation_negative_values.transpose(), presynaptic_negative_values)
        indices_to_be_nudge_down = ~indices_to_be_nudge_up

        update_amount = (0.001 * cp.dot(neurons_activation.transpose(), neurons_stress))
        layer_axons -= cp.where(indices_to_be_nudge_down, update_amount, 0)
        layer_axons -= cp.where(indices_to_be_nudge_up, update_amount, 0)

    def backward_axons_and_dentrites_update(neurons_activation, neurons_stress, layer_axons, layer_dentrites):
        indices_to_be_nudge_up = cp.dot(neurons_activation.transpose(), neurons_stress) > 0
        indices_to_be_nudge_down = ~indices_to_be_nudge_up
    ...
    def update_axons_and_dentrites(neurons_and_stress, axons_to_update: list, dentrites_to_update: list, for_backward_pass: bool=False):
        ...
        for layer_idx in range(len(network_architecture)-1):
            ...
            axons = axons_to_update[-(layer_idx+1)]
            if not for_backward_pass:
                forward_axons_and_dentrites_update(neurons_activation, neurons_stress, axons, None)
            else:
                backward_axons_and_dentrites_update(neurons_activation, neurons_stress, axons, None)
